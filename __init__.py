...
def bone_depth(b):
    """Return the number of ancestors a bone has (used for sorting parent-first)."""
    depth = 0
    p = b.parent
    while p:
        depth += 1
...
#            wigglebones.append(b)
        ...
        # Sort parent-first so physics are evaluated in correct hierarchy order
        wigglebones.sort(key=bone_depth)
...
def update_prop(self,context,prop): 
    ...
    if type(self) == bpy.types.PoseBone:
        ...
        for b in context.selected_pose_bones:
            if b == self: continue  # skip self, already changed, avoid re-trigger
            if getattr(b, prop) != new_val:
                setattr(b, prop, new_val)
...
def get_parent(b):
    ...
    # Parent must be active in wiggle list (has wiggle.matrix initialized by move())
    # If parent is enabled but NOT in the list, treat it as non-wiggle parent to avoid
    # child reading a zero/stale wiggle.matrix from parent.
    armature_entry = bpy.context.scene.wiggle.list.get(b.id_data.name)
    parent_in_list = armature_entry and armature_entry.list.get(p.name) is not None
    par = p if (p.wiggle_enable and (not p.wiggle_mute) and parent_in_list and ((p.wiggle_head and not p.bone.use_connect) or p.wiggle_tail)) else get_parent(p)
    return par
...
#                sy = sy*(b.parent.length/b.parent.bone.length)
    ...
    if last:
        # Only use the matrix_basis path when a constraint actively drives orientation.
        # LIMIT_* constraints merely clamp values and don't need this path — using it
        # incorrectly drops the parent world transform, causing the child to ignore parent rotation.
        ORIENTATION_CONSTRAINT_TYPES = {
            'DAMPED_TRACK','TRACK_TO','LOCKED_TRACK','STRETCH_TO',
            'IK','SPLINE_IK','COPY_ROTATION','COPY_TRANSFORMS',
            'COPY_LOCATION','CHILD_OF','FLOOR','FOLLOW_PATH',
            'PIVOT','SHRINKWRAP','ACTION',
        }
        const = False
        for c in b.constraints:
            if c.enabled and c.type in ORIENTATION_CONSTRAINT_TYPES:
                const = True
                break
        if const:
...
@persistent        
def wiggle_render_pre(scene, depsgraph=None):
    scene.wiggle.is_rendering = True
    
@persistent
def wiggle_render_post(scene, depsgraph=None):
    scene.wiggle.is_rendering = False
    
@persistent
def wiggle_render_cancel(scene, depsgraph=None):
    scene.wiggle.is_rendering = False
    
@persistent
def wiggle_load(scene, depsgraph=None):
    build_list()
    s = bpy.context.scene
...
class WiggleBake(bpy.types.Operator):
    ...
    def execute(self,context):
        ...
        while preroll >= 0:
            ...
            preroll -= 1
        #bake
        # Blender 4.1+ requires channel_types argument; Blender 5.x also uses this path
...
class WiggleBone(bpy.types.PropertyGroup):
    ...
    collision_col: bpy.props.BoolProperty(default=False, override={'LIBRARY_OVERRIDABLE'})  # flag: collider refs initialized
